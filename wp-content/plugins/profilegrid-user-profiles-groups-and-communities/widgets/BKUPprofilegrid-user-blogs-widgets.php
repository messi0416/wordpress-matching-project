...
class Profilegrid_User_Blogs extends WP_Widget {
    ...
    public function widget($args,$instance)
    {
        ...
        if (!empty($posts)):
            /* カスタマイズ 2021/5/19 */
            echo '<ul class="custom_new_list">';
            foreach ($posts as $post):

                echo '<div class="custom_new_item">';

                /* 日付の設定 */
                $cutamdate = mysql2date('Y年m月d日', $post->post_date);
                echo '<div class="custom_new_item_date">'. $cutamdate.'</div> ';
                
                echo '<div class="custom_new_item_body">';
                echo '<div class="custom_new_item_tag_area">';
                    /* タグの取得・設定 */
                    $tags = get_the_tags($post->ID);
                    if($tags):
                        foreach ( $tags as $tag ):
                            echo '<span class="custom_new_item_tag tag-link-' . $tag->term_id. '">' . $tag->name . '</span>';
                        endforeach;
                    else:
                        echo '<span class="custom_new_item_tag_not"></span>';
                    endif;

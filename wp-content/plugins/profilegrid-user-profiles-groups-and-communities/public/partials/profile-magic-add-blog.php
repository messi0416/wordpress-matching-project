...
$settings = array(
  'wpautop' => false, 'media_buttons' => false, 'textarea_name' => 'blog_description',
...
if (isset($_POST['blog_title'])) {
  ...
  // mes-stop enter the '$_POST' in '$post' variable
  if (!isset($post['blog_tags'])) $post['blog_tags'] = '';

  // Add by cravel start
  /**
   * タグが複数指定に対応していないため機能修正
   * 
   * @since  1.0.0
   * @author Cravel <cravel@cravelweb.com>
   */
  $blog_tags = unserialize($post['blog_tags']);
  //mes-stop-getting-prefecture-&-status
  $allowed_ext = 'jpg|jpeg|png|gif';
  $arg = array(
    'post_type' => 'profilegrid_blogs',
    'post_title' => $post['blog_title'],
    'post_name' => shortCRC32($post['blog_title'].date("Y-m-d H:i:s")), // スラッグをハッシュ値に変換して一意にする
    'post_status' => $poststatus,
    'tags_input'    => $blog_tags,
  ...
  if ($postid) {
    ...
    // add by Cravel start
    matching_post_custom_fields($post, $postid); //ブログ投稿時にカスタムフィールドを登録する
    // add by Cravel end

...
if(isset($_POST['edit_profile']))
{
	...
	$gids = $pmrequests->profile_magic_get_user_field_value($_POST['euid'],'pm_group');
        ...
        if(!empty($gid_array))
        {
            $exclude = "associate_group in(".implode(',',$gid_array).") and field_type not in('user_name','user_email','user_avatar','user_pass','confirm_pass','paragraph','heading','term_checkbox')";
...
if(isset($content['id']))$uid = $content['id'];
else $uid = filter_input(INPUT_GET, 'uid');
if(isset($uid))
{

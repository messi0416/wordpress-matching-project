...
function start_messenger() {
  ...
  jQuery("#receipent_field").autocomplete({
    ...
    source: function (request, response) {
      ...
      autocomplete_request = jQuery.post(
        pm_chat_object.ajax_url,
        data,
        function (resp) {
          if (resp) {
  ...
  jQuery("#message_display_area").scroll(function () {
    ...
    if (jQuery("#load_more_message").length) {
      ...
      if (
        jQuery("#message_display_area").offset().top - 100 <=
        jQuery("#load_more_message").offset().top
      ) {
  ...
  jQuery(document).ready(function () {
    ...
    jQuery(".pmagic #unread_notification_count").css(
      "background-color",
      pmDomColor
    );
    jQuery(
      ".pmagic .pm-blog-desc-wrap #chat_message_form input#receipent_field"
    ).css("color", pmDomColor);
    jQuery(".pmagic .pm-new-message-area button").css("color", pmDomColor);
    jQuery(".pmagic .pm-messenger-button svg").css("fill", pmDomColor);
    jQuery(
      ".pmagic .pm-thread-active .pm-conversations-container .pm-thread-user"
    ).css("color", pmDomColor);
    jQuery(".pm-color").css("color", pmDomColor);
    jQuery("#pg-friends .pm-selected-image svg").css("fill", pmDomColor);
    jQuery(".pmagic .page-numbers .page-numbers.current")
      .addClass("pm-bg")
      .css("background", pmDomColor);
    jQuery(".pm-group-view.pg-theme-seven .pg-profile-area-wrap").css(
      "background",
      pmDomColor
    );
    ...
    jQuery(".pmagic .pm-profile-tab-wrap .pm-profile-tab").hover(
      ...
      function () {
        ...
        jQuery(
          ".pm-section-nav-horizental .pm-profile-tab.ui-state-active"
        ).css("border-bottom-color", pmDomColor);
      }
    );
  });
}
...
function pm_messenger_send_chat_message(event) {
  ...
  var html =
    '<div id="" class="pm_msg_rf  pm-sending-msg" >' +
    img +
  ...
  jQuery("#message_display_area").scrollTop(1300000);
}
...
function show_thread_messages(tid, loadnum) {
  ...
  //console.log("offset is "+offset);
  var data = {
    action: "pm_messenger_show_messages",
    tid: tid,
    loadnum: loadnum,
    timezone: offset,
  };
  ...
  jQuery.post(pm_chat_object.ajax_url, data, function (resp) {
    ...
    if (jQuery("#thread_pane").length) {
      if (loadnum == "1") {
        jQuery("#message_display_area").html(resp);
        jQuery("#message_display_area").scrollTop(130000);
      } else {
        jQuery("#message_display_area").prepend(resp);
        jQuery("#message_display_area").scrollTop(
          jQuery("#load_more_message").offset().top + 500
        );
      }
    }
  });
...
function pm_get_messenger_notification(timestamp, activity) {
  ...
  notification_request = jQuery.get(
    pm_chat_object.ajax_url,
    data,
    function (response) {
      if (response) {
